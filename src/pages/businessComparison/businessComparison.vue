    ...
    <div class="businessComparison" v-loading="loading">
        ...
        <div class="right">
            ...
            <div class="con" :style="{ height: content_height }">
                <div style="height: 100%" v-show="!switchShow">
                    <my-table-base
                            height="100%"
                            :pagination="false"
                            chooseItem="single"
                ...
                <div class="charts" v-show="switchShow">
                    <div class="content" v-for="i in 3" :key="i">
                        <div class="title" v-if="i===1">
                            {{selected['饼图(左)']}}饼图(左)
                        </div>
                        <div class="title" v-if="i===2">
                            {{selected['饼图(右)']}}饼图(右)
                        </div>
    ...
    export default {
        ...
        methods: {
            ...
            lookView() {
                ...
                this.$axios.get("/api/service_show/get_comprehensive_data", {params: param}).then(res => {
            ...
            this.$axios.get("/api/service_show/get_comprehensive_data", {params: param}).then(res => {
                ...
                this.$nextTick(_ => {
                    for (let i = 1; i < 4; i++) {
                        let id = "aabc" + i;
                        this.$echarts.init(document.getElementById(id)).dispose();
                        this.$echarts.init(document.getElementById(id)).setOption(option[i - 1]);
                    }
                    this.loading = false;
                });
            });
