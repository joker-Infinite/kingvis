    ...
    <div class="financialChart" v-loading="loading">
        ...
        <div class="financialChart_right">
            <div class="financialChart_right_header">
                ...
                <span style="width: 20px;display: inline-block"></span>
                <el-select v-model="pageType" @change="changePageType">
                    <el-option label="全部" :value="null"></el-option>
                    <el-option label="收入" :value="1"></el-option>
                    <el-option label="支出" :value="2"></el-option>
                </el-select>
                <span style="width: 20px;display: inline-block"></span>
                <el-select v-model="year" @change="selectYears">
            ...
            <div class="financialChart_right_content">
                <div id="dimension_container"
                     :class="{dimension_container:true,dimension_container_height:!visible,dimension_container_min_height:visible}">
                    <div class="dimension">
                        <el-checkbox-group v-model="select" @change="selectCheckBox">
                            <template v-for="i in latitude">
                                <el-checkbox :label="i['kmId']" :value="i['kmId']">{{i['kmName']}}</el-checkbox>
                            </template>
                        </el-checkbox-group>
                    </div>
                    <div class="dimension_btn">
                        <el-button type="text" :disabled="this.latitude.length===0" @click="showOffAll">选择纬度</el-button>
                    </div>
                </div>
                <div class="container" :style="{height:content_height}">
                    <div class="table_container" v-show="!switchShow">
                        <my-table-base height="calc(100% - 50px)"
                                       :search="false"
                                       chooseItem="single"
                                       :columns="columns"
                                       :tableData="tableData"
    ...
    export default {
        ...
        data() {
            return {
                ...
                columns: [
                    /*{
                      prop: 'yiJiflowingKmmc', label: '一级科目', width: '200px', formatter: v => {
                          if (!v['yiJiflowingKmmc']) {
                              return 0;
                          }
                          return v['yiJiflowingKmmc'];
                      }
                  },*/
                    {
                        prop: 'flowingKmmc', label: '科目名称', formatter: v => {
                            if (!v['flowingKmmc']) {
                                return 0;
                            }
                    ...
                    {
                        prop: 'flowingTitle', label: '流水名称', formatter: v => {
                            if (!v['flowingTitle']) {
                                return 0;
                            }
                    ...
                    /* {
                         prop: 'payType', label: '支出方式', formatter: v => {
                             if (!v['payType']) {
                                 return 0;
                             }
                             return v['payType'];
                    ...
                    {
                        prop: 'flowingMoney', label: '金额', formatter: v => {
                            if (!v['flowingMoney']) {
                                return 0;
                            }
                    ...
                    {
                        prop: 'deptName', label: '单位名称', formatter: v => {
                            if (!v['deptName']) {
                                return 0;
                            }
                            return v['deptName'];
        ...
        methods: {
            ...
            showOffAll() {
                ...
                setTimeout(_ => {
                    let height = document.getElementById('dimension_container').offsetHeight;
                    this.content_height = "calc(100% - " + height + "px)";
                    this.loading = false;
                }, 500)
            },
            async common(v) {
                ...
                this.option[0].series[0].data = km_;
                this.option[1].series[0].data = gs_;
                this.option[2].series[0].data = this.mySet(sr, 'yAxis');
                this.option[2].series[1].data = this.mySet(zc, 'yAxis');
                this.option[2].xAxis[0].data = xb_;
