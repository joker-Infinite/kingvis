  ...
  >
    <div class="work_report_header">
      ...
      <div class="work_report_header_title">
        <p class="mainTitle">
          湖北交投实业公司2021年{{ new Date().getMonth() + 1 }}月重点工作汇报
        </p>
        <!--<p class="subtitle">
      ...
      <div class="typeSelect">
        <div
          :class="{ typeBtn: true, btnImg: btnImg === i }"
          v-for="i in 4"
          @click="typeSelect(i)"
        >
          {{
    ...
    <div class="work_report_content">
      <el-table-base
        ...
        @allData="
          v => {
            (this.table = v), (this.isLoading = false), this.initECharts(v);
          }
        "
        :btnImg="btnImg"
...
export default {
  ...
  methods: {
    nextUnit(v) {
      ...
      this.forIndex = arr;
      this.thisImg = v;
      this.headerBorderImg = require("../../assets/report/header/headerBorder-" +
        v +
        ".png");
      this.getAllData();
    },
    ...
    initECharts(v) {
      for (let i = 1; i < 5; i++) {
        let item = this.$echarts["init"](
          document.getElementById("footer-" + i)
        );
        if (item.dispose()) {
          item.dispose();
        }
      ...
      let [yzwdb, wdb, db, ce, jd] = [0, 0, 0, 0, []];
      ...
      v.forEach(i => {
        ...
        //年度重点工作与时间进度对比
        {
          if (i["progressRate"] - schedule < -2000) {
            yzwdb++;
          }
          if (
            i["progressRate"] - schedule >= -2000 &&
            i["progressRate"] - schedule < 0
          ) {
            wdb++;
          }
          if (
          ...
          if (i["subProject"] && i["subProject"].length > 0) {
            ...
            subProject.forEach(j => {
              if (j["progressRate"] - schedule < -2000) {
                yzwdb++;
              }
              if (
                j["progressRate"] - schedule >= -2000 &&
                j["progressRate"] - schedule < 0
              ) {
                wdb++;
              }
              if (
    ...
    async getAllData() {
      ...
      const unitData = (
        await this.$axios["get"](
          "https://okr.hbjtsy.com:7080/jt/zt/getDeptList"
        )
      ).data.data;
      ...
      const d = await this.$axios["get"]("/wj/report/getMonthReport", {
        params: {
          ...
          reportTime: "2021-05-01 00:00:00"
        }
      });
