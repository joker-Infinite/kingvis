<template>
    <div class="public_pie" v-loading="loading">
        <public_title></public_title>
        <div class="pieContent" :id="dataID"></div>
    </div>
</template>

<script>
    import "./public_pie.less"
    ...
    export default {
        ...
        methods: {
            /**
             * 获取该图表数据
             * @param url 获取数据的接口并拼接代理
             * @param params 接口参数
             * */
            async getChartData(url, params) {
                let data
                if (url) {
             ...
             * */
            async setECharts(d, v, s) {
                this.dataID = d[0]["name"] + '-' + d[0]["value"];
        ...
        async mounted() {
            let data = await this.getChartData(this.url, this.params);
